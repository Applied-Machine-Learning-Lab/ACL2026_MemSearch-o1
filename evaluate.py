...
def evaluate_predictions(dataset_name, output, labeled_answer, mode='gen'):
    ...
    if mode == 'qa':
        if dataset_name in ['multifieldqa_zh']:
            for answer in labeled_answer:
                normalized_pred_answer = normalize_zh_answer(pred_answer)
                normalized_ground_truth = normalize_zh_answer(answer)
                prediction_tokens = list(jieba.cut(normalized_pred_answer, cut_all=False))
                ground_truth_tokens = list(jieba.cut(normalized_ground_truth, cut_all=False))
            ...
            for answer in labeled_answer:
                ...
                if num_same == 0:
                    continue
                f1 = rouge_zh_score(normalized_pred_answer, normalized_ground_truth)
                for k in ["em", "acc", "f1"]:
                    final_metric[k] = max(eval(k), final_metric[k])
...
#         }
    ...
    if dataset_name != 'livecode':
        ...
        for item, input_prompt, result in zip(filtered_data, input_list, output_list):
            ...
            if dataset_name in ['gpqa', 'medmcqa']:
                ...
                mode = 'gen'
            elif dataset_name in ['hotpotqa', 'musique', 'bamboogle', '2wikimqa', 'narrativeqa', 'qasper', 'multifieldqa_en', 'multifieldqa_zh', 'dureader']:
            ...
            metric, pred_answer = evaluate_predictions(dataset_name, output=item['Output'], labeled_answer=labeled_answer, mode=mode)
            item['Pred_Answer'] = pred_answer
            item['Metrics'] = metric
            item['Question'] = input_prompt

            # Determine the validity of the predicted answer
            my_method_valid = (pred_answer != '' and not (mode == 'choose' and dataset_name == 'gpqa' and len(pred_answer) > 0))
    ...
    t = time.localtime()
    result_json_name = f'{split}.json'
    metrics_json_name = f'{split}_metrics.json'
    if apply_backoff:
...
if __name__ == "__main__":
    ...
    if 'gpqa' in output_path:
        ...
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif '2wikimqa' in output_path:
        dataset_name = '2wikimqa'
        normal_output_path = './outputs/runs.qa/2wikimqa.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
        ...
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif 'qasper' in output_path:
        dataset_name = 'qasper'
        normal_output_path = './outputs/runs.qa/qasper.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif 'narrativeqa' in output_path:
        dataset_name = 'narrativeqa'
        normal_output_path = './outputs/runs.qa/narrativeqa.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif 'multifieldqa_en' in output_path:
        dataset_name = 'multifieldqa_en'
        normal_output_path = './outputs/runs.qa/multifieldqa_en.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif 'multifieldqa_zh' in output_path:
        dataset_name = 'multifieldqa_zh'
        normal_output_path = './outputs/runs.qa/multifieldqa_zh.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
    elif 'dureader' in output_path:
        dataset_name = 'dureader'
        normal_output_path = './outputs/runs.qa/dureader.qwq.direct/test.12.15,15:32.json'
        if 'qwq' not in output_path:
            normal_output_path = ''
    ...
    if dataset_name != 'livecode':
        ...
        for i, item in enumerate(data):
            if dataset_name in ['gpqa', 'medmcqa']:
                ...
                mode = 'gen'
                domain = 'Unknown'
            elif dataset_name in ['narrativeqa', 'qasper', 'multifieldqa_en', 'hotpotqa', 'musique', '2wikimqa', 'multifieldqa_zh', 'dureader']:
                labeled_answer = item["answer"]
                mode = 'qa'
                domain = 'Unknown'
            elif dataset_name in ['pubhealth']:
